...
import curatedEmojiList from "../../../data/emoji-list";
...
// Generated by lib/generateImages.ts — do not edit by hand.
const emojiToKey: Record<string, string> = emojiMap;

// The curated originals a random request should *land* on. Every glyph is
// filtered through emojiToKey so a missing asset can never leak into the pool.
// (The supporting "run through a bunch of different ones" animation still draws
// from the full emojiToKey set below.)
const landingEmojiList = curatedEmojiList.filter((emoji) => emojiToKey[emoji]);
...
function parseEmoji(input: string): string[] {
  if (input === "random") {
    ...
    return sampleSize(landingEmojiList, count);
  }
